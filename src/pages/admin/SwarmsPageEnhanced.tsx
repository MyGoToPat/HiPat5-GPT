...
type PromptsShape = 'id' | 'key' | 'unknown';
...
export default function SwarmsPageEnhanced() {
  ...
  // Detect prompts schema shape on mount
  useEffect(() => {
  ...
  const handleSaveManifest = async () => {
    ...
    try {
      ...
      const errorMsg = e.message.includes('JSON') || e.message.includes('parse')
  ...
  const handlePublish = async (versionId: string) => {
    ...
    try {
      await swarmsAPI.publishSwarmVersion(versionId);
  ...
  const handleRolloutChange = async (versionId: string, percent: number) => {
    ...
    try {
      await swarmsAPI.updateRollout(versionId, { rollout_percent: percent, cohort: cohortValue });
  ...
  const handleSavePromptDraft = async () => {
    ...
    const { agentKeyOrId, model, text, title } = editingPrompt;

    if (!agentKeyOrId?.trim() || !text?.trim()) {
      setPromptError('Agent identifier and content are required');
      toast.error('Please fill in all required fields');
      return;
    }

    const MAX_PROMPT_SIZE = 50000;
    if (text.length < 1 || text.length > MAX_PROMPT_SIZE) {
      setPromptError(`Content must be between 1 and ${MAX_PROMPT_SIZE} characters (currently ${text.length})`);
      toast.error('Content length invalid');
      return;
    }

    try {
      const payload = toPromptWritePayload({
        agentKeyOrId: agentKeyOrId.trim(),
        model: model || 'gpt-4o-mini',
        text: text.trim(),
        title: title?.trim() || undefined,
      }, promptsShape);

      const result = await swarmsAPI.createPromptDraft(payload as any);

      console.debug('[enhanced-swarms] prompts: createDraft ok', { id: result.data?.id, agent: agentKeyOrId });
  ...
  const handlePublishPrompt = async (id: string) => {
    ...
    try {
      await swarmsAPI.publishPrompt(id);
  ...
  const promptsShapeDisplay = promptsShape === 'unknown' ? 'unknown(key default)' : promptsShape;
              ...
              <div className="mt-3 flex items-center gap-3">
                ...
                <div className="inline-flex items-center gap-2 px-3 py-2 bg-gray-100 border border-gray-300 rounded text-xs font-mono">
                  ...
                  <span className="text-gray-700">
                    ...
                    promptsShape={promptsShapeDisplay}
                  </span>
                </div>
              </div>
              {!WRITE_ENABLED && (
                  ...
                  <div className="flex items-start justify-between">
                    ...
                    <div className="relative group">
                      <button
                        ...
                        title={!WRITE_ENABLED ? "Writes disabled in this environment" : ""}
                      >
                  ...
                  <div className="p-6">
                      ...
                      {!editingPrompt ? (
                        <div className="relative group">
                          <button
                            onClick={() => {
                              if (WRITE_ENABLED) {
                                setEditingPrompt({ agentKeyOrId: '', model: 'gpt-4o-mini', text: '', title: '' });
                              }
                            }}
                            disabled={!WRITE_ENABLED}
                            aria-disabled={!WRITE_ENABLED}
                    ...
                    {editingPrompt ? (
                      <div className="space-y-4">
                        <div>
                          <label className="block text-sm font-medium text-gray-700 mb-2">
                            Agent Key/ID <span className="text-red-500">*</span>
                          </label>
                          <input
                            type="text"
                            value={editingPrompt.agentKeyOrId}
                            onChange={(e) => {
                              if (WRITE_ENABLED) {
                                setEditingPrompt({ ...editingPrompt, agentKeyOrId: e.target.value });
                                setPromptError('');
                              }
                            }}
                            readOnly={!WRITE_ENABLED}
                      ...
                      <div className="space-y-3">
                        {agentPrompts.length === 0 ? (
                          ...
                          agentPrompts.map((row: any) => {
                            const prompt = fromPromptRow(row);
                            return (
                              <div
                                key={prompt.id}
                                className={`border rounded-lg p-4 ${
                                ...
                                <div className="flex items-start justify-between mb-2">
                                  <div className="flex-1">
                                    <div className="flex items-center gap-2 mb-1">
                                      <span className="font-medium text-gray-900">{prompt.title || prompt.agentIdentifier}</span>
                                      <span
                                        className={`text-xs px-2 py-1 rounded ${
                                          prompt.status === 'published'
                                            ? 'bg-green-200 text-green-800'
                                    ...
                                    <p className="text-xs text-gray-600">
                                      <span className="font-medium">Agent:</span> {prompt.agentIdentifier} | <span className="font-medium">Model:</span> {prompt.model}
                                    </p>
                                    <p className="text-xs text-gray-600 mt-1">
