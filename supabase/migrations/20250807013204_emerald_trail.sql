/*
  ...
  1. New Tables
    ...
    - `user_metrics`
      - `id` (uuid, primary key)
      - `user_id` (uuid, foreign key to auth.users)
      - `bmr` (integer, optional)
      - `tdee` (integer, optional)
    ...
    - `chat_messages`
      - `id` (uuid, primary key)
      - `chat_history_id` (uuid, foreign key to chat_histories)
      - `text` (text, required)
      - `timestamp` (timestamp)
    ...
    - `food_logs`
      ...
      - `grams` (numeric, required)
      - `source_db` (text, default 'OpenAI LLM')
      - `macros` (jsonb, required)
      - `created_at` (timestamp)
    ...
    - `role_change_history`
      - `id` (uuid, primary key)
      - `user_id` (uuid, foreign key to auth.users)
      - `old_role` (text, required)
      - `new_role` (text, required)
  ...
  2. Helper Functions
    ...
    - `log_role_change()` - Logs role changes to history table

  3. Security
    - Enable RLS on all tables
    - Role-based access policies for admin and regular users
