/*
  # Fix nutrition resolver RPC - resolve ambiguous column references

  1. Changes
    - Prefix all local variables with v_ to avoid ambiguity with table columns
    - Fix WHERE clause to properly reference variables vs columns
    - Ensure correct comparison between function parameters and table data

  2. Testing
    - Should now resolve food items without ambiguous column errors
*/
...
BEGIN
  ...
  LOOP
    ...
    SELECT jsonb_build_object(
      'kcal', COALESCE(fc.calories_per_100g * v_grams_used / 100, 0),
      'protein_g', COALESCE(fc.protein_per_100g * v_grams_used / 100, 0),
      'carbs_g', COALESCE(fc.carbs_per_100g * v_grams_used / 100, 0),
      'fat_g', COALESCE(fc.fat_per_100g * v_grams_used / 100, 0)
    )
    INTO macros
    FROM public.food_cache fc
    WHERE lower(fc.food_name) = lower(v_food_name)
    LIMIT 1;
    
    -- If no cache hit, use generic estimates
