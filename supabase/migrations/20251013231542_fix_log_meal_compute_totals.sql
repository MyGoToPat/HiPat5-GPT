/*
  # Fix log_meal RPC - Compute totals before insert

  1. Changes
    - Update log_meal RPC to compute totals from p_items
    - Insert computed totals into meal_logs (NOT NULL constraint)
    - Ensures totals JSONB has both kcal and calories keys

  2. Security
    - Keeps existing SECURITY DEFINER and grants
*/

-- Drop and recreate with totals computation
...
begin
  ...
  select jsonb_build_object(
    'kcal', coalesce(sum((item->>'energy_kcal')::numeric), 0),
    'calories', coalesce(sum((item->>'energy_kcal')::numeric), 0),
    'protein_g', coalesce(sum((item->>'protein_g')::numeric), 0),
    'fat_g', coalesce(sum((item->>'fat_g')::numeric), 0),
    'carbs_g', coalesce(sum((item->>'carbs_g')::numeric), 0),
    'fiber_g', coalesce(sum((item->>'fiber_g')::numeric), 0)
  )
  into v_totals
  from jsonb_array_elements(p_items) as item;

  -- Insert meal log with computed totals
  insert into meal_logs (user_id, ts, meal_slot, note, totals)
  values (v_user_id, coalesce(p_ts, now()), v_meal_slot, p_note, v_totals)
